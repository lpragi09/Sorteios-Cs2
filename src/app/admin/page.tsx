...
export default function AdminDashboard() {
  ...
  const carregarDadosCompletos = () => {
    ...
    let sorteios: Sorteio[] = JSON.parse(salvos || "[]");
    if (sorteios.length === 0) {
        sorteios = [{ id: "ak47", nome: "AK-47 | Redline", img: "https://steamcommunity-a.akamaihd.net/economy/image/-9a81dlWLwJ2UUGcVs_nsVtzdOEdtWwKGZZLQHTxDZ7I56KU0Zwwo4NUX4oFJZEHLbXH5ApeO4YmlhxYQknCRvN0_rTKQXw/360fx360f", valor: "150,00", status: "Ativo" }];
        localStorage.setItem("lista_sorteios", JSON.stringify(sorteios));
    }
    ...
    sorteios.forEach(s => {
        const tickets = JSON.parse(localStorage.getItem(`tickets_${s.id}`) || "[]");
  ...
  const abrirSorteio = (sorteio: Sorteio) => {
    ...
    setTicketsDoSorteio(JSON.parse(localStorage.getItem(`tickets_${sorteio.id}`) || "[]"));
    setListaGanhadores(JSON.parse(localStorage.getItem(`ganhadores_${sorteio.id}`) || "[]"));
  };

  const iniciarSorteio = () => {
    if (!sorteioSelecionado) return;
    const ticketsAprovados: Ticket[] = JSON.parse(localStorage.getItem(`tickets_${sorteioSelecionado.id}`) || "[]").filter((t: Ticket) => t.status === "Aprovado");
    ...
    const loopSorteio = () => {
        ...
        if (interacoes < maxInteracoes) {
            ...
            const novaLista = [...listaGanhadores, novoG];
            setListaGanhadores(novaLista);
            localStorage.setItem(`ganhadores_${sorteioSelecionado.id}`, JSON.stringify(novaLista));
        }
    };
    loopSorteio();
  };
  ...
  const handleToggleStatus = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    const nova = listaSorteios.map(s => s.id === id ? { ...s, status: s.status === "Ativo" ? "Finalizado" : "Ativo" } : s) as Sorteio[];
    setListaSorteios(nova); localStorage.setItem("lista_sorteios", JSON.stringify(nova)); carregarDadosCompletos();
  };

  const handleDeletarSorteio = (e: React.MouseEvent, id: string) => {
    e.stopPropagation(); if (!confirm("Excluir sorteio?")) return;
    const nova = listaSorteios.filter(s => s.id !== id);
    setListaSorteios(nova); localStorage.setItem("lista_sorteios", JSON.stringify(nova)); carregarDadosCompletos();
  };

  const validarTicket = (id: number, status: string) => {
    const nova = ticketsDoSorteio.map(t => t.id === id ? { ...t, status } : t);
    setTicketsDoSorteio(nova); if (sorteioSelecionado) localStorage.setItem(`tickets_${sorteioSelecionado.id}`, JSON.stringify(nova)); carregarDadosCompletos();
  };
  ...
  const salvarEdicaoTicket = (e: React.FormEvent) => {
    ...
    setTicketsDoSorteio(nova); localStorage.setItem(`tickets_${sorteioSelecionado.id}`, JSON.stringify(nova)); carregarDadosCompletos(); setTicketEmEdicao(null);
  };
  ...
  const handleCriarSorteio = (e: React.FormEvent) => { e.preventDefault(); if (!formImg) return; const novo = { id: Date.now().toString(), nome: formNome, img: formImg, valor: formValor, status: "Ativo" } as Sorteio; const novaLista = [...listaSorteios, novo]; setListaSorteios(novaLista); localStorage.setItem("lista_sorteios", JSON.stringify(novaLista)); setModalCriarAberto(false); limparForm(); carregarDadosCompletos(); };
  ...
  const handleSalvarEdicaoSorteio = (e: React.FormEvent) => { e.preventDefault(); if (!sorteioEmEdicao) return; const nova = listaSorteios.map(s => s.id === sorteioEmEdicao.id ? { ...s, nome: formNome, valor: formValor, img: formImg } : s) as Sorteio[]; setListaSorteios(nova); localStorage.setItem("lista_sorteios", JSON.stringify(nova)); setSorteioEmEdicao(null); carregarDadosCompletos(); };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-4 md:p-8 overflow-x-hidden">
      <div className="max-w-7xl mx-auto">
                    ...
                    <div className="animate-in slide-in-from-right-4 duration-500">
                        ...
                        {listaGanhadores.length > 0 && (
                            <div className="mt-8 bg-slate-900 rounded-2xl border border-slate-800 p-6 animate-in fade-in duration-700">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {listaGanhadores.map(g => (
                                        <div key={g.ticket.id} className="bg-slate-950 p-4 rounded-2xl border border-green-900/30 flex items-center gap-4 relative overflow-hidden group hover:border-green-500/50 transition">
                                            ...
                                            <img src={g.ticket.userImage || "https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png"} alt="" className="w-14 h-14 rounded-full border-2 border-yellow-500/50 object-cover group-hover:scale-110 transition" />
                                            <div>
                                ...
                                <button onClick={() => { if(confirm("Limpar histórico de ganhadores?")) { localStorage.removeItem(`ganhadores_${sorteioSelecionado.id}`); setListaGanhadores([]); } }} className="mt-6 text-[10px] text-slate-500 hover:text-red-500 font-bold flex items-center gap-1 uppercase tracking-widest transition"><RefreshCw className="w-3 h-3"/> Resetar Sessão</button>
                            </div>
                        )}
                    </div>
                )}
                ...
                <div className="w-full max-w-2xl text-center relative z-10">
                    {sorteando ? (
                        ...
                        <div className="space-y-8 animate-in fade-in zoom-in-50 duration-1000 ease-out flex flex-col items-center">
                            ...
                            <button onClick={() => setModalSorteioAberto(false)} className="mt-10 group relative px-16 py-5 bg-white text-black rounded-full font-black text-2xl hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,255,255,0.2)] active:scale-95">CONTINUAR SESSÃO</button>
                        </div>
                    ) : null}
                </div>
            </div>
            ...
            <div className="fixed inset-0 bg-black/90 z-[1000] flex items-center justify-center p-4 backdrop-blur-md animate-in zoom-in-95">
                <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-slate-800 p-8 shadow-2xl relative">
                    ...
                    <form onSubmit={handleCriarSorteio} className="space-y-5">
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block ml-1">Nome da Skin</label><input type="text" required value={formNome} onChange={e => setFormNome(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white focus:border-yellow-500 outline-none transition-all" placeholder="Ex: M4A4 | Howl" /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block ml-1">Valor de Mercado</label><input type="text" required value={formValor} onChange={e => setFormValor(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white focus:border-yellow-500 outline-none transition-all" placeholder="Ex: 1.200,00" /></div>
        ...
        {sorteioEmEdicao && (
            <div className="fixed inset-0 bg-black/90 z-[1000] flex items-center justify-center p-4 backdrop-blur-md animate-in zoom-in-95">
                <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-blue-500/20 p-8 shadow-2xl relative">
                    <button onClick={()=>setSorteioEmEdicao(null)} className="absolute top-6 right-6 text-slate-500 hover:text-white"><X/></button>
                    <h3 className="font-black text-2xl mb-6 text-white uppercase tracking-tight flex items-center gap-2"><Pencil className="text-blue-500"/> Editar</h3>
                    <form onSubmit={handleSalvarEdicaoSorteio} className="space-y-5">
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block ml-1">Nome</label><input type="text" value={formNome} onChange={e => setFormNome(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white focus:border-blue-500 outline-none" /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block ml-1">Valor</label><input type="text" value={formValor} onChange={e => setFormValor(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white focus:border-blue-500 outline-none" /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block ml-1">Imagem</label><div className="border-2 border-dashed border-slate-800 rounded-2xl p-6 text-center relative hover:bg-slate-950 transition cursor-pointer"><input type="file" accept="image/*" onChange={handleImagemChange} className="absolute inset-0 opacity-0 cursor-pointer" />{formImg ? <img src={formImg} alt="" className="h-24 mx-auto" /> : <Upload className="mx-auto text-slate-500"/>}</div></div>
                        <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 py-5 rounded-2xl font-black text-white text-lg transition shadow-lg active:scale-95 uppercase tracking-tighter">Salvar Alterações</button>
                    </form>
                </div>
            </div>
        )}
        ...
        {ticketEmEdicao && (
            <div className="fixed inset-0 bg-black/90 z-[1000] flex items-center justify-center p-4 backdrop-blur-md animate-in zoom-in-95">
                <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-yellow-500/20 p-8 shadow-2xl relative">
                    <button onClick={()=>setTicketEmEdicao(null)} className="absolute top-6 right-6 text-slate-500 hover:text-white"><X/></button>
                    <h3 className="font-black text-2xl mb-6 text-white uppercase tracking-tight flex items-center gap-2"><Pencil className="text-yellow-500"/> Ajustar Ticket</h3>
                    <form onSubmit={salvarEdicaoTicket} className="space-y-5">
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">ID CSGOBIG</label><input type="text" value={ticketEmEdicao.csgobigId} onChange={(e) => setTicketEmEdicao({...ticketEmEdicao, csgobigId: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white outline-none" /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Quantidade Coins</label><input type="number" value={ticketEmEdicao.coins} onChange={(e) => setTicketEmEdicao({...ticketEmEdicao, coins: Number(e.target.value)})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-yellow-500 font-black" /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Nome Instagram</label><input type="text" value={ticketEmEdicao.instagram} onChange={(e) => setTicketEmEdicao({...ticketEmEdicao, instagram: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-white outline-none" /></div>
                        <button type="submit" className="w-full bg-blue-600 py-5 rounded-2xl font-black text-white text-lg transition shadow-lg active:scale-95">SALVAR MUDANÇAS</button>
                    </form>
                </div>
            </div>
        )}

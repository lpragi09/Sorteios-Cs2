...
// Ícones importados (incluindo os do Rodapé)
import { 
  Shield, Users, Gift, CheckCircle, XCircle, ExternalLink, 
  Plus, Pencil, X, Upload, Trash2, Coins, BarChart3, Trophy, 
  RefreshCw, Lock, Unlock, TrendingUp, Sparkles, Zap, 
...
export default function AdminDashboard() {
  ...
  const carregarDadosCompletos = async () => {
    ...
    if (sorteiosError) { 
        console.error("Erro ao carregar sorteios:", sorteiosError); 
        return; 
    }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0f1014]">
        ...
        {/* ESPAÇAMENTO AUMENTADO: pt-40 para garantir que saia de baixo da navbar fixa */}
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-10 border-b border-white/5 pb-6">
                <h1 className="text-3xl font-black text-white flex items-center gap-2 italic uppercase">
            ...
            {abaAtiva === "dashboard" && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-[#1b1e24] p-6 rounded-2xl border border-white/5 flex items-center gap-4 transition hover:border-yellow-500/50 hover:bg-[#1b1e24]/80">
                            <div className="bg-yellow-500/10 w-14 h-14 rounded-xl flex items-center justify-center text-yellow-500"><Gift /></div>
                            <div><h3 className="text-slate-400 text-xs font-bold uppercase tracking-widest">Sorteios Ativos</h3><p className="text-3xl font-black">{totalSorteiosAtivos}</p></div>
                        </div>
                        <div className="bg-[#1b1e24] p-6 rounded-2xl border border-white/5 flex items-center gap-4 transition hover:border-blue-500/50 hover:bg-[#1b1e24]/80">
                            <div className="bg-blue-500/10 w-14 h-14 rounded-xl flex items-center justify-center text-blue-500"><Users /></div>
                            <div><h3 className="text-slate-400 text-xs font-bold uppercase tracking-widest">Entradas Ativas</h3><p className="text-3xl font-black">{totalEntradasAtivas}</p></div>
                        </div>
                        <div className="bg-[#1b1e24] p-6 rounded-2xl border border-white/5 flex items-center gap-4 transition hover:border-green-500/50 hover:bg-[#1b1e24]/80">
                            <div className="bg-green-500/10 w-14 h-14 rounded-xl flex items-center justify-center text-green-500"><Coins /></div>
                            <div><h3 className="text-slate-400 text-xs font-bold uppercase tracking-widest">Pool Ativo</h3><p className="text-3xl font-black text-green-400">{totalCoinsAtivos}</p></div>
                        </div>
                    </div>
                    <div>
                        <h2 className="text-xl font-bold mb-4 flex items-center gap-2 border-l-4 border-blue-500 pl-3 uppercase italic"><TrendingUp className="text-blue-500"/> Detalhamento</h2>
                        <div className="space-y-3">
                            {listaSorteios.map((s) => (
                                <div key={s.id} className="bg-[#1b1e24] p-4 rounded-xl border border-white/5 flex flex-col md:flex-row md:items-center justify-between hover:bg-[#15171c] transition gap-4">
                                    <div className="flex items-center gap-4">
                                        <img src={s.img} className="w-12 h-12 object-contain bg-[#0f1014] rounded-lg p-1 border border-white/5" />
                                        <div>
                                            <h3 className="font-bold text-white text-lg italic uppercase">{s.nome}</h3>
                                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${s.status === 'Ativo' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>{s.status}</span>
                                        </div>
                                    </div>
                                    <div className="flex gap-8 text-right justify-between md:justify-end">
                                        <div><p className="text-[10px] text-slate-400 uppercase tracking-widest">Entradas</p><p className="text-xl font-bold">{statsDetalhadas[s.id]?.entries || 0}</p></div>
                                        <div><p className="text-[10px] text-slate-400 uppercase tracking-widest">Coins</p><p className="text-xl font-bold text-yellow-500">{statsDetalhadas[s.id]?.coins || 0}</p></div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {!sorteioSelecionado ? (
                        <div>
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold border-l-4 border-yellow-500 pl-3 uppercase italic">Gerenciamento</h2>
                                <button onClick={() => { limparForm(); setModalCriarAberto(true); }} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 px-6 py-2 rounded-lg font-bold transition shadow-lg shadow-green-900/20 uppercase text-sm"><Plus className="w-5 h-5"/> Novo Sorteio</button>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {listaSorteios.map((s) => (
                                    <div key={s.id} onClick={() => abrirSorteio(s)} className={`bg-[#1b1e24] p-6 rounded-2xl border cursor-pointer hover:border-yellow-500 transition group relative flex flex-col sm:flex-row sm:items-center gap-6 ${s.status === "Finalizado" ? "border-red-900/50 opacity-80" : "border-white/5"}`}>
                                        <img src={s.img} className="w-24 h-24 object-contain bg-[#0f1014] rounded-xl p-2 transition group-hover:scale-110 border border-white/5" />
                                        <div className="flex-1">
                                            <h3 className="text-xl font-black text-white group-hover:text-yellow-500 transition italic uppercase">{s.nome}</h3>
                                            <p className="text-slate-400 text-sm">R$ {s.valor}</p>
                                            {s.status === "Finalizado" && <span className="text-red-500 font-bold text-[10px] uppercase mt-1 inline-block border border-red-500 px-2 rounded">Encerrado</span>}
                                        </div>
                                        <div className="flex gap-2 z-10">
                                            <button onClick={(e) => handleToggleStatus(e, s.id)} className={`p-2.5 rounded-lg shadow-lg transition ${s.status === "Ativo" ? "bg-[#0f1014] text-yellow-500 hover:bg-yellow-600 hover:text-white border border-white/5" : "bg-red-600 text-white hover:bg-red-500"}`}>{s.status === "Ativo" ? <Unlock className="w-5 h-5" /> : <Lock className="w-5 h-5" />}</button>
                                            <button onClick={(e) => handleAbrirEdicaoSorteio(e, s)} className="p-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-500"><Pencil className="w-5 h-5" /></button>
                        ...
                        <div className="animate-in slide-in-from-right-4 duration-500">
                            ...
                            <div className="bg-[#1b1e24] rounded-2xl border border-white/5 overflow-hidden shadow-2xl">
                                <div className="p-6 border-b border-white/5 bg-[#1b1e24] flex flex-col sm:flex-row justify-between items-center gap-4">
                                    <h2 className="text-xl font-bold text-white flex gap-2 items-center italic uppercase"><BarChart3 className="text-yellow-500"/> Entradas: {sorteioSelecionado.nome}</h2>
                                    <button onClick={iniciarSorteio} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white px-8 py-3 rounded-xl font-black shadow-lg shadow-green-900/40 transition-all hover:scale-105 active:scale-95 uppercase italic"><Sparkles className="w-5 h-5"/> SORTEAR AGORA</button>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm text-slate-400">
                                        <thead className="bg-[#15171c] text-slate-200 uppercase font-bold text-[10px]">
                                            <tr>
                                        ...
                                        <tbody className="divide-y divide-white/5">
                                            {ticketsDoSorteio.length === 0 ? (
                                                <tr><td colSpan={7} className="p-8 text-center text-slate-500 italic">Nenhum ticket encontrado.</td></tr>
                                            ) : (
                                                ticketsDoSorteio.map((t) => (
                                                    <tr key={t.id} className="hover:bg-white/5 transition">
                                                        <td className="p-4"><div>{t.data.split(",")[0]}</div><div className="text-[10px] text-slate-500">{t.email}</div></td>
                                                        ...
                                                        <td className="p-4 text-center"><a href={t.print} target="_blank" className="bg-[#0f1014] hover:bg-white/10 text-white px-3 py-1.5 rounded-lg text-[10px] inline-flex items-center gap-1 border border-white/10 shadow-sm transition">Abrir <ExternalLink className="w-3 h-3"/></a></td>
                                                        <td className="p-4 text-center"><span className={`px-3 py-1 rounded-full text-[10px] font-black border uppercase tracking-wide ${t.status === "Aprovado" ? "bg-green-500/10 text-green-400 border-green-500/20" : t.status === "Rejeitado" ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-yellow-500/10 text-yellow-500 border-yellow-500/20"}`}>{t.status}</span></td>
                                                        <td className="p-4 flex justify-center gap-2">
                                                            <button onClick={() => setTicketEmEdicao(t)} className="p-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg shadow-md transition active:scale-90"><Pencil className="w-4 h-4" /></button>
                                                            <button onClick={() => validarTicket(t.id, "Aprovado")} className="p-2 bg-green-600 hover:bg-green-500 text-white rounded-lg shadow-md transition active:scale-90"><CheckCircle className="w-4 h-4" /></button>
                                                            <button onClick={() => validarTicket(t.id, "Rejeitado")} className="p-2 bg-red-600 hover:bg-red-500 text-white rounded-lg shadow-md transition active:scale-90"><XCircle className="w-4 h-4" /></button>
                                                        </td>
                                                    </tr>
                                                ))
                                            )}
            ...
            {/* MODAIS CRIAR/EDITAR (FUNDO ESCURO E BORDAS CORRIGIDAS) */}

...
// Adicionei AlertCircle e outros ícones necessários
import { Shield, Users, Gift, CheckCircle, XCircle, ExternalLink, Plus, Pencil, X, Upload, Trash2, Coins, BarChart3, Trophy, RefreshCw, Lock, Unlock, TrendingUp, Sparkles, Zap, Eye, AlertCircle } from "lucide-react";
...
type Ticket = {
    ...
    data: string;
    csgobigId?: string;
    csgobig_id?: string; // Compatibilidade com banco
    coins: number;
    instagram: string;
    print_url?: string;
    print?: string;
...
// Tipo visual para a lista de ganhadores na tela
type GanhadorVisual = {
    round: number;
    ticket: Ticket;
    dataGanhou: string;
...
export default function AdminDashboard() {
  ...
  const [formImgUrl, setFormImgUrl] = useState(""); // URL para edição
  ...
  const carregarDadosCompletos = async () => {
    // 1. Busca do Supabase em vez do LocalStorage
  ...
  const abrirSorteio = async (sorteio: Sorteio) => {
    ...
    // 3. Busca Ganhadores do Supabase
  ...
  // --- LÓGICA DO SORTEIO ÉPICO ---
  const iniciarSorteio = () => {
    if (!sorteioSelecionado) return;
    
    const ticketsAprovados = ticketsDoSorteio.filter(t => t.status === "Aprovado");
    const idsGanhadores = listaGanhadores.map(g => g.ticket.id);
    const candidatos = ticketsAprovados.filter(t => !idsGanhadores.includes(t.id));
    ...
    const loopSorteio = () => {
        ...
        if (interacoes < maxInteracoes) {
            ...
            salvarGanhadorNoBanco(vencedorReal);
        }
    };
    loopSorteio();
  };

  const salvarGanhadorNoBanco = async (ticket: Ticket) => {
      if(!sorteioSelecionado) return;
  ...
  const handleSalvarEdicaoSorteio = async (e: React.FormEvent) => {
      ...
      try {
          ...
          // Se trocou a imagem, faz upload da nova
          if (formImgFile) {
            const fileExt = formImgFile.name.split('.').pop();
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-4 md:p-8 overflow-x-hidden">
      <div className="max-w-7xl mx-auto">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-center gap-4 mt-4">
                                    ...
                                    <div className="px-6 py-3 bg-slate-900 border border-slate-800 rounded-2xl flex items-center gap-3">
                                        ...
                                        <span className="text-lg font-bold text-slate-300 uppercase tracking-tighter">{ganhadorRevelado.csgobigId || ganhadorRevelado.csgobig_id}</span>
                                    </div>
                                </div>
                            </div>
        ...
        {/* MODAIS CRIAR/EDITAR (CORRIGIDOS PARA SUPABASE) */}

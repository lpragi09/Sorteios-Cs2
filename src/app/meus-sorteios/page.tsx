...
import { Ticket, CheckCircle, Search, Home, ChevronLeft, ChevronRight, AlertCircle } from "lucide-react";
...
const ITENS_POR_PAGINA = 12; // Carrega 12 tickets por vez (leve)
  ...
  const carregarMeusTickets = async (pagina: number) => {
    ...
    try {
      ...
      const { data, error, count } = await supabase
        ...
        `, { count: 'exact' }) // Pede a contagem total pra saber quantas páginas tem
        .eq('email', session?.user?.email)
        .order('id', { ascending: false })
        .range(from, to); // A MÁGICA: Só busca do X ao Y
